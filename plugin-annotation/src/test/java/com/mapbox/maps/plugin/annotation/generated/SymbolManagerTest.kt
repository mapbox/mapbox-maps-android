...
class SymbolManagerTest {
  ...
  fun initialize() {
    ...
    assertEquals(Symbol.ID_KEY, manager.getAnnotationIdKey())
    verify { style.addLayer(any()) }
    manager = SymbolManager(mapView, delegateProvider, AnnotationConfig("test_layer"))
  ...
  fun click() {
    ...
    val manager = SymbolManager(mapView, delegateProvider)
    val annotation = manager.create(
      SymbolOptions()
        .withPoint(Point.fromLngLat(0.0, 0.0))
    )
    ...
    val listener = mockk<OnSymbolClickListener>()
  ...
  fun longClick() {
    ...
    val listener = mockk<OnSymbolLongClickListener>()
  ...
  fun drag() {
    ...
    val listener = mockk<OnSymbolDragListener>()
