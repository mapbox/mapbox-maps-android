...
class MapOverlayPluginTest {
  ...
  private val cameraManagerDelegate = mockk<CameraManagerDelegate>()
  ...
  fun getReframeCameraOption() {
    ...
    every {
      cameraManagerDelegate.cameraForCoordinateBounds(
        capture(slot),
        capture(paddingSlot),
        any(),
        any()
    ...
    every { cameraManagerDelegate.setCamera(capture(cameraSlot)) } just Runs
    mapOverlayPlugin.reframe()
    mapOverlayPlugin.reframe {
