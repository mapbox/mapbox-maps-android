...
/**
 * Fire-and-forget animation function for Pitch as [Double]. When the provided [targetValue] is changed, the
 ...
 *
 * [animatePitchAsState] returns a [State] object. The value of the state object will continuously
 * be updated by the animation until the animation finishes.
 *
 * Note, [animatePitchAsState] cannot be canceled/stopped without removing this composable function
...
/**
 * Creates a Pitch animation as a part of the given [Transition]. This means the states
...
@Composable
@MapboxExperimental
public inline fun Transition<CameraState>.animatePitch(
  noinline transitionSpec:
  @Composable Transition.Segment<CameraState>.() -> FiniteAnimationSpec<Double> = { defaultDoubleAnimation },
  label: String = "CameraPitchAnimation",
  targetValueByState: @Composable (state: CameraState) -> Double = { it.pitch }
