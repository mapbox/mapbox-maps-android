  ...
  fun <T> createPlugin(
    ...
    if (!plugins.containsKey(pluginImplementationClazz)) {
      ...
      if (instance is ContextBinder) {
        instance.bind(mapboxMapOptions.context, mapboxMapOptions.attrs, mapboxMapOptions.pixelRatio)
      }
  ...
  fun onCameraMove(cameraState: CameraState) {
    cameraPlugins.forEach {
      it.onCameraMove(
        ...
        cameraState.bearing,
        arrayOf(
          cameraState.padding.left,
          cameraState.padding.top,
          cameraState.padding.right,
          cameraState.padding.bottom
        )
      )
    }
  }
