 ...
 */
class MapboxViewportDataSourceOptions internal constructor() {
  /**
...
class FollowingFrameOptions internal constructor() {
   ...
   * the puck will not be tied to the bottom edge of the [MapboxViewportDataSource.followingPadding] and instead move
   ...
   * to maximize the view of that geometry within the [MapboxViewportDataSource.followingPadding].
   *
   * Defaults to `true`.
   */
  ...
  class IntersectionDensityCalculation internal constructor() {
    /**
     * **Preconditions**:
     * - a route is provided via [MapboxViewportDataSource.onRouteChanged]
     * - updates are provided via [MapboxViewportDataSource.onRouteProgressChanged]
     *
  ...
  class PitchNearManeuvers internal constructor() {
    ...
    /**
     * When this option is enabled and the preconditions are met,
     * this variable describes the threshold distance to the next maneuver makes the frame with pitch `0`, based on the [MapboxViewportDataSource.onRouteProgressChanged].
     *
  ...
  class BearingSmoothing internal constructor() {
    /**
     * If enabled, the **following frame**'s bearing won't exactly reflect the bearing returned
     * by the [Location] from [MapboxViewportDataSource.onLocationChanged] but will also be
     * affected by the direction to the upcoming framed geometry, to maximize the viewable area.
     *
     * Defaults to `true`.
     *
    ...
    /**
     * When enabled, this controls how much the **following frame**'s bearing can
     * deviate from the [Location] bearing, in degrees.
     *
     * Defaults to `45.0` degrees.
     */
...
class OverviewFrameOptions internal constructor() {
   ...
   *
   * Simplifying geometries, especially for longer routes, can have a significant impact on the performance of generating frames and each [MapboxViewportDataSource.evaluate] calls.
   */
