...
* A fix of the layer paint property evaluation while transitioning from a data-driven value. It snaps immediately to the new value thus preventing of drawing stale data during the animation.
* Reduced memory consumption when using raster layers by deleting CPU side tile bitmap copy after uploading to GPU texture.
