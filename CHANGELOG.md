...
* Refine experimental `WidgetPosition` API by unifying the `margin`/`translation` into the `WidgetPosition` class via builder pattern, as well as renamed `horizontal`/`vertical` to `horizontalAlignment`/`verticalAlignment`. The original constructors and `setTranslation` API are deprecated. ([1782](https://github.com/mapbox/mapbox-maps-android/pull/1782))
